...
public class CommonTreeParser {
	...
	public void constructTDS(Tree tree, SymbolTable tds) {
		...
		switch (tree.getText()) {
			...
			case "BLOCK":
				this.tds.putLink(tree.getChild(0).getText(), new SymbolTable(this.tds.getImbricationLevel() + 1, this.tds));
				break;

			case "THEN":
				this.tds.putLink(tree.getChild(0).getText(), new SymbolTable(this.tds.getImbricationLevel() + 1, this.tds)); //Certainly not working
				break;
